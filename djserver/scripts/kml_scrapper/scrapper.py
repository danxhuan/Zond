...
def get_kml_regions(kml_path: str, tif_path: str, forced=True) -> str:
    ...
    if not os.path.isdir(kml_path) or not os.path.isfile(tif_path):
        raise OSError
    # Выходная директория на уровень выше (родительская папка)
    parent_dir = os.path.dirname(os.path.dirname(tif_path))  # Дважды вызываем, чтобы подняться на 2 уровня
    res_path = os.path.join(parent_dir, 'regions')  # Или просто '..', если нужен относительный путь
